  ...
  async function createBufferKeymap(doc) {
    ...
    for (let p of pairs2) {
      if (Array.isArray(p) && p.length == 2) {
        ...
        subscriptions.push(
          import_coc2.workspace.registerExprKeymap("i", character, insertPair.bind(null, character, matched), true)
        );
        if (matched != character) {
          subscriptions.push(import_coc2.workspace.registerExprKeymap("i", matched, closePair.bind(null, matched), true));
        }
      }
    }
    nvim.resumeNotification(false, true);
  }
  void createBufferKeymap(import_coc2.workspace.getDocument(import_coc2.workspace.bufnr));
